...
template <typename T> class LockFreeList {
  ...
  struct AMR {
    ...
    bool compare_and_set(Node *expected_ref, Node *new_ref, bool expected_mark,
                         bool new_mark) {
      auto expected = pack(expected_ref, expected_mark);
      return raw.compare_exchange_strong(expected, pack(new_ref, new_mark),
  ...
  struct Node {
    ...
    Node(std::optional<T> i, int k, Node *n)
        : item(std::move(i)), key(k), next(n, false) {}
  };
